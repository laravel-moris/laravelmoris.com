        ...
        <div class="max-w-2xl mx-auto">
            <x-ui.card class="p-8">
                ...
                <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data" class="mt-6 space-y-6">
                    ...
                    <div>
                        <label for="bio" class="text-[12px] font-bold uppercase tracking-[0.14em] text-muted">Bio</label>
                        <textarea id="bio" name="bio" rows="4" class="w-full rounded-2xl bg-surface-2 border border-border/70 px-4 py-3 text-[15px] text-foreground placeholder:text-muted/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 focus-visible:border-primary/60">{{ old('bio', auth()->user()->bio) }}</textarea>
                        @error('bio')
                    ...
                    <div>
                        <label for="avatar" class="text-[12px] font-bold uppercase tracking-[0.14em] text-muted">Avatar</label>
                        <input
                            type="file"
                            id="avatar"
                            name="avatar"
