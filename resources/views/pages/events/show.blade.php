            ...
            <x-ui.card class="p-6 mb-6">
                ...
                <div class="flex flex-wrap items-center gap-4 text-sm mb-6">
                    <div class="flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5 text-muted shrink-0">
                    ...
                    @if($event->location)
                        <div class="flex items-center gap-2">
                            ...
                            @endif
                            <a href="{{ $event->location->directions_url ?? ($event->location->url ?? '#') }}" target="_blank" rel="noopener noreferrer" class="border-b border-dotted border-current hover:text-primary transition-colors cursor-pointer">
                                @if($event->type->value === 'physical')
                    ...
                    <div class="flex items-center gap-3">
                        @auth
                            ...
                            <form action="{{ route('events.rsvp', $event) }}" method="POST">
                                ...
                                <x-ui.button
                                    type="submit"
                                    variant="{{ $isMaybe ? 'primary' : 'secondary' }}"
                                >
                ...
                <x-ui.card class="p-6 mb-6">
                    <div class="flex items-center justify-center gap-6 flex-wrap">
                        @foreach($event->sponsors as $sponsor)
                            <a href="{{ route('sponsors.show', $sponsor) }}" class="block transition-opacity hover:opacity-80 p-3 bg-surface-2 rounded-lg">
                                @if($sponsor->logo)
                                    <img
                                        src="{{ Storage::disk('public')->url($sponsor->logo) }}"
                                        alt="{{ $sponsor->name }}"
                        ...
                        <div class="space-y-3">
                            @foreach($event->speakers as $speaker)
                                <a href="{{ route('members.show', $speaker) }}" class="block group">
                                    <div class="p-4 bg-surface-2 rounded-lg transition-all duration-300 hover:border-primary/50 border border-transparent">
                                        {{-- Talk Title - Main Focus --}}
                                        <x-ui.text.body class="font-semibold text-lg mb-3 group-hover:text-primary transition-colors">
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="min-w-0">
                                                ...
                                                @if($speaker->title)
                                                    <x-ui.text.muted class="text-xs truncate">{{ $speaker->title }}</x-ui.text.muted>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
