                ...
                <div class="lg:col-span-1">
                    <x-ui.card class="p-6">
                        <div class="flex flex-col items-center text-center">
                            <div class="w-full h-48 rounded-xl bg-white/5 border border-border/70 overflow-hidden mb-4 flex items-center justify-center p-6">
                                <img
                                    src="{{ $logoUrl }}"
                                    alt="{{ $sponsor->name }}"
                            ...
                            @if($sponsor->events->count() > 0)
                                <x-ui.text.muted class="mt-2">
                                    {{ $sponsor->events->count() }} {{ Str::plural('event', $sponsor->events->count()) }} sponsored
                                </x-ui.text.muted>
                            @endif
                            ...
                            <div class="mt-4 space-y-3">
                                @foreach($sponsor->events as $event)
                                    <a href="{{ route('events.show', $event) }}" class="block group">
                                        <div class="flex items-center justify-between p-4 bg-surface-2 rounded-lg transition-all duration-300 group-hover:border-primary/50 border border-transparent">
                                            <div>
