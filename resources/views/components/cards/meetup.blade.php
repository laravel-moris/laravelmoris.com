...
<a href="{{ route('events.show', $card->id) }}" class="block {{ $rootClasses }}" data-reveal>
    ...
    <div
        class="inline-flex flex-col items-center rounded-2xl border border-border/70 px-5 py-4 transition-transform duration-300 {{ $featured ? 'bg-coral border-transparent scale-[1.05]' : 'bg-surface-2 group-hover:scale-[1.05] group-hover:bg-coral group-hover:border-transparent' }}">
        <span
            class="text-[11px] font-bold uppercase tracking-[0.14em] {{ $featured ? 'text-white/85' : 'text-muted group-hover:text-white/85' }}">{{ $month }}</span>
        <span
            class="mt-1 text-[32px] font-bold leading-none {{ $featured ? 'text-white' : 'text-foreground group-hover:text-white' }}">{{ $day }}</span>
    </div>
    ...
    <div class="mt-5 flex border-t border-border/70 pt-5">
        @foreach ($speakers as $speaker)
            <div
                class="size-10 rounded-full bg-surface-2 border-[3px] border-surface -ml-3 first:ml-0 overflow-hidden transition-transform duration-300 group-hover:-translate-y-1 hover:z-10 hover:-translate-y-1.5 hover:scale-[1.05]">
